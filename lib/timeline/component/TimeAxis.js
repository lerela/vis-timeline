 ...
 */
  ...
  _repaintMajorLine(left, width, orientation, className) {
    ...
    line.style.width = `${width}px`;
    line.style.height = `${props.majorLabelHeight}px`;

    let y = (orientation == 'top') ? 0 : this.body.domProps.top.height;
    let x = left - props.majorLineWidth / 2;

    this._setXY(line, x, y);
    line.className = `vis-grid ${this.options.rtl ?  'vis-vertical-rtl' : 'vis-vertical'} vis-major vis-major-topper`;

    // reuse redundant line
    line = this.dom.redundant.lines.shift();
    if (!line) {
      // create vertical line
      line = document.createElement('div');
      this.dom.background.appendChild(line);
    ...
    y = (orientation == 'top') ? props.majorLabelHeight : this.body.domProps.top.height;
    x = left - props.majorLineWidth / 2;
