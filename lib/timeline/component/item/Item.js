 ...
 */
  ...
  /**
   * Delete the drag area on the center of the item
   * @protected
   */
  _deleteDragCenter() {
    if (!this.dom.dragCenter) return;
   ...
   */
  _repaintDragCenter() {
    const editable = ((this.options.editable.overrideItems || this.editable == null) && this.options.editable.updateTime) ||
      (!this.options.editable.overrideItems && this.editable != null && this.editable.updateTime);

    if (this.selected && !this.dom.dragCenter) {
      const me = this;
      ...
      this.hammerDragCenter.on('doubletap', event => {
        ...
        me.parent.itemSet._onDoubleClickItem(me);
  ...
  */
  ...
  _updateStyle(element) {
    let callbackItemStyleFunction;
    // remove old styles
    if (this.style) {
      util.removeCssText(element, this.style);
      this.style = null;
    }
    if (this.options.callbackItemStyle) {
      callbackItemStyleFunction = this.options.callbackItemStyle.bind(this);
      let customStyle = util.xss(callbackItemStyleFunction(this.data, element));
      util.addCssText(element, customStyle);
